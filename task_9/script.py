...
def do_test(matrix_size):
    ...
    for _ in range(factor):
        proc = subp.run(["./execs/method_1", f"{matrix_size}"], input=input_string, capture_output=True)
        method1.append(float(proc.stdout.decode()))

    for _ in range(factor):
        proc = subp.run(["./execs/method_2", f"{matrix_size}"], input=input_string, capture_output=True)
        method2.append(float(proc.stdout.decode()))

    for _ in range(factor):
        proc = subp.run(["./execs/method_3", f"{matrix_size}"], input=input_string, capture_output=True)
    ...
    print("\tRecursive algorithm:  {:.2f} <=> {:.2f} <=> {:.2f}".format(stat.mean(method2), stat.harmonic_mean(method2), stat.pstdev(method2)))
